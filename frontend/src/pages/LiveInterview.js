...
import {
  Video, VideoOff, Mic, MicOff, SkipForward,
  CheckCircle, Loader2, UserCircle, Maximize, Timer
} from 'lucide-react';
...
export default function LiveInterview() {
  ...
  // Prep timer: after TTS ends, counts down before auto-recording
  ...
  const prepCountdownRef = useRef(0);
  ...
  useEffect(() => {
    const onFsChange = () => {
      ...
      if (!isFull && mountedRef.current && interview) {
        // User exited fullscreen — abort interview
        toast.error('Interview aborted — you exited fullscreen mode.');
        (async () => {
          try {
            await axios.post(`${API}/interviews/${interviewId}/abort`, {}, { headers: getAuthHeaders(), withCredentials: true });
          } catch {}
          navigate('/dashboard');
        })();
      }
  ...
  const initMedia = useCallback(async () => {
    ...
    try {
      ...
      setMediaError(err.name === 'NotAllowedError' ? 'Camera/microphone access denied.' : err.name === 'NotFoundError' ? 'No camera or microphone found.' : 'Unable to access camera/microphone.');
      setMediaReady(false);
    }
  }, []);
  ...
  const speakQuestion = useCallback((text) => {
    ...
    utterance.onend = () => {
      setIsSpeaking(false);
      // After speaking ends, start prep countdown (only if question not already answered)
      if (mountedRef.current) startPrepTimer();
    };
    utterance.onerror = () => {
      setIsSpeaking(false);
      if (mountedRef.current) startPrepTimer();
  ...
  }, []); // startPrepTimer added below via ref

  // ==================== PREP TIMER (after TTS) ====================
  const startPrepTimerRef = useRef(null);

  const startPrepTimer = useCallback(() => {
    // Don't start if already recording, preparing, or question already answered
    if (isRecordingRef.current || !mediaReady) return;

    // Clear any existing prep timer
    if (prepTimerRef.current) { clearInterval(prepTimerRef.current); prepTimerRef.current = null; }

    setIsPreparing(true);
    prepCountdownRef.current = timeLimit;
    setPrepCountdown(timeLimit);

    prepTimerRef.current = setInterval(() => {
      prepCountdownRef.current -= 1;
      const val = prepCountdownRef.current;
      setPrepCountdown(val);
      if (val <= 0) {
  ...
  }, [timeLimit, mediaReady]);

  // Keep the ref updated
  startPrepTimerRef.current = startPrepTimer;

  // Override speakQuestion's onend to use the ref
  const speakQuestionWrapped = useCallback((text) => {
    if (!text || !window.speechSynthesis) return;
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.rate = 0.95;
    ...
    utterance.onend = () => {
      setIsSpeaking(false);
      if (mountedRef.current && startPrepTimerRef.current) startPrepTimerRef.current();
    };
    utterance.onerror = () => {
      setIsSpeaking(false);
      if (mountedRef.current && startPrepTimerRef.current) startPrepTimerRef.current();
  ...
  // Speak when question changes (only when fullscreen is active)
  useEffect(() => {
    if (!voicesReady || !interview?.questions?.[currentIndex] || !isFullscreen) return;
    if (lastSpokenIndexRef.current === currentIndex) return;
    // Don't speak if already answered
    if (answeredQuestions.has(currentIndex)) return;
    lastSpokenIndexRef.current = currentIndex;
    const t = setTimeout(() => speakQuestionWrapped(interview.questions[currentIndex]), 400);
    return () => clearTimeout(t);
  }, [currentIndex, interview, speakQuestionWrapped, voicesReady, isFullscreen, answeredQuestions]);
    ...
    try {
      ...
      if (interview) {
        ...
        setAnsweredQuestions(prev => {
          ...
          if (nextIdx >= 0) {
            lastSpokenIndexRef.current = -1; // Reset so TTS speaks next question
  ...
  const actuallyStartRecording = useCallback(() => {
    ...
    if (!vt || vt.readyState === 'ended' || !at || at.readyState === 'ended') { toast.error('Media stream ended.'); setMediaReady(false); initMedia(); return; }
  ...
  // Manual start recording (skips prep timer)
  const startRecording = () => {
    if (!streamRef.current || !mediaReady) { toast.error('Camera/mic not available.'); return; }
    // Cancel any running prep timer
    if (prepTimerRef.current) { clearInterval(prepTimerRef.current); prepTimerRef.current = null; }
    setIsPreparing(false);
    setPrepCountdown(0);
    actuallyStartRecording();
  ...
  const completeInterview = async () => {
    ...
    try {
      ...
      exitFullscreen();
      // Small delay so fullscreen exit doesn't trigger abort
      mountedRef.current = false;
      setTimeout(() => navigate(`/interview/${interviewId}/review`), 300);
    } catch { toast.error('Failed to complete interview'); setCompleting(false); }
  };
  ...
  const prepPct = isPreparing ? (prepCountdown / timeLimit) * 100 : 0;

  // ==================== FULLSCREEN GATE ====================
  if (!isFullscreen) {
    return (
      <div ref={containerRef} className="min-h-screen flex items-center justify-center bg-background" data-testid="fullscreen-gate">
        <Card className="max-w-md border shadow-lg">
          <CardContent className="p-8 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-xl font-bold font-heading">Fullscreen Required</h2>
              <p className="text-sm text-muted-foreground leading-relaxed">
                The interview runs in fullscreen mode to simulate a real interview environment. If you exit fullscreen, the interview will be automatically aborted.
              </p>
            </div>
            <Button size="lg" className="gap-2 w-full" onClick={enterFullscreen} data-testid="enter-fullscreen-btn">
              <Maximize className="w-4 h-4" /> Enter Fullscreen &amp; Start
            </Button>
            <Button variant="ghost" size="sm" onClick={() => navigate('/dashboard')} data-testid="cancel-interview-btn">
              Cancel
            </Button>
          </CardContent>
        </Card>
  ...
  // ==================== MAIN INTERVIEW UI ====================
  return (
    <div ref={containerRef} className="min-h-screen bg-background" data-testid="live-interview-page">
      {/* Top Bar */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {isPreparing && (
                <div className="flex items-center gap-2" data-testid="prep-timer-badge">
                  <Timer className="w-4 h-4 text-amber-500" />
                  <span className="text-sm font-mono text-amber-500 font-semibold">{formatTime(prepCountdown)}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid lg:grid-cols-2 gap-4 mb-6">
          ...
          <Card className="border overflow-hidden" data-testid="interviewer-panel">
            <div className="relative aspect-[4/3] bg-zinc-900">
              <img src={INTERVIEWER_IMG} alt="AI Interviewer" className="w-full h-full object-cover object-top" />
              {isSpeaking && (
          ...
          <Card className="border overflow-hidden" data-testid="video-preview">
            <div className="relative aspect-[4/3] bg-black">
              ...
              {/* Prep timer overlay on camera */}
              {isPreparing && !isRecording && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/40" data-testid="prep-overlay">
                  <div className="text-center space-y-3">
                    <Timer className="w-10 h-10 text-amber-400 mx-auto" />
                    <p className="text-3xl font-bold font-mono text-white">{formatTime(prepCountdown)}</p>
                    <p className="text-sm text-white/70">Prepare your answer...</p>
                    <p className="text-xs text-white/50">Recording starts automatically</p>
                  </div>
                </div>
              )}
              {isRecording && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  {isPreparing && <Badge variant="outline" className="text-[10px] gap-1 text-amber-500 border-amber-500/30">Preparing {formatTime(prepCountdown)}</Badge>}
                </div>
                <p className="text-base font-medium leading-relaxed">{questions[currentIndex]}</p>
                {isPreparing && (
                  <div className="space-y-1 pt-1">
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-1000 ease-linear bg-amber-500" style={{ width: `${prepPct}%` }} />
                    </div>
                    <p className="text-[10px] text-muted-foreground text-right">Recording starts in {formatTime(prepCountdown)}</p>
                  </div>
                )}
                {isRecording && (
                  <div className="space-y-1 pt-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {questions.map((_, i) => (
              <Button key={i} variant={i === currentIndex ? 'default' : answeredQuestions.has(i) ? 'secondary' : 'outline'} size="sm" className="w-9 h-9 p-0 text-xs font-mono" onClick={() => { if (!isRecording && !isPreparing) { lastSpokenIndexRef.current = -1; setCurrentIndex(i); setCountdown(0); setPrepCountdown(0); setIsPreparing(false); if (prepTimerRef.current) { clearInterval(prepTimerRef.current); prepTimerRef.current = null; } } }} disabled={isRecording || isPreparing} data-testid={`question-nav-${i}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            {!mediaReady && !isRecording ? (
              ...
              <Button size="lg" className="gap-2 rounded-full px-6 bg-amber-500 hover:bg-amber-600 text-white" onClick={startRecording} disabled={savingResponse} data-testid="record-now-btn">
                Record Now (skip wait)
              </Button>
            ) : !isRecording ? (
              <Button size="lg" className="gap-2 rounded-full px-6 bg-red-500 hover:bg-red-600 text-white" onClick={startRecording} disabled={isCurrentAnswered || savingResponse || isSpeaking} data-testid="start-recording-btn">
                {savingResponse ? <Loader2 className="w-4 h-4 animate-spin" /> : `Record (${formatTime(timeLimit)})`}
              </Button>
            ) : (
            ...
            <Button variant="outline" size="icon" className="w-10 h-10 rounded-full" onClick={nextQuestion} disabled={isRecording || isPreparing || isLastQuestion} data-testid="next-question-btn"><SkipForward className="w-4 h-4" /></Button>
          </div>
        </div>

        <p className="text-xs text-muted-foreground text-center mt-4">
          {!mediaReady ? 'Click "Enable Camera" to start' : isSpeaking ? 'Sarah is asking the question...' : isPreparing ? 'Prepare your answer. Recording starts automatically when timer ends.' : isRecording ? 'Click "Stop & Submit" to finish early, or wait for auto-submit' : isCurrentAnswered ? 'This question is answered. Navigate to another.' : `Sarah will ask the question, then you'll get ${formatTime(timeLimit)} to prepare before recording starts.`}
        </p>
      </div>
    </div>
  );
