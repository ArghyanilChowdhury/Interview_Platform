...
export default function LiveInterview() {
  ...
  // Start camera/mic - handles StrictMode double-mount
  ...
  const startRecording = () => {
    if (!streamRef.current || !mediaReady) {
      toast.error('Camera/mic not available. Click "Enable Camera" to retry.');
      return;
    }
    ...
    if (!videoTrack || videoTrack.readyState === 'ended' || !audioTrack || audioTrack.readyState === 'ended') {
      toast.error('Media stream ended. Reinitializing...');
      setMediaReady(false);
      initMedia();
      return;
    }
    ...
    // Start MediaRecorder with supported mimeType
    try {
    ...
    try {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onerror = () => {};
        recognition.onend = () => {
          // Use ref to check current recording state (avoids stale closure)
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="sticky top-24">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-xs text-zinc-400">
                          {mediaError ? 'Permission needed' : 'Camera off'}
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <p className="text-xs text-muted-foreground text-center mt-3">
                {!mediaReady
                  ? 'Click "Enable Camera" to start recording'
                  : isRecording
