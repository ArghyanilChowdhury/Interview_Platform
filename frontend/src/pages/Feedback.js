...
export default function Feedback() {
  ...
  const handleSubmit = (e) => {
    ...
    setRating(0); setFeedbackText(''); setEmail('');
  };
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10" data-testid="feedback-page">
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="your@email.com — so we can follow up" value={email} onChange={e => setEmail(e.target.value)} data-testid="feedback-email" />
            </div>

            <Button type="submit" className="gap-2" data-testid="feedback-submit">
      ...
      <div className="flex items-start gap-3 p-4 rounded-md bg-muted/50 border">
        ...
        <p className="text-sm text-muted-foreground">
          Your feedback directly shapes our roadmap. Every suggestion is read by our team and prioritized based on community impact.
        </p>
      </div>
    </div>
  );
